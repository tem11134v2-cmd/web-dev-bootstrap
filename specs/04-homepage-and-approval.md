...
Собрать главную страницу из секций под бизнес-задачу заказчика, выкатить на dev-окружение (или прямо на prod при схеме A), показать заказчику, собрать правки, итерировать до одобрения. Без одобрения главной — НЕ переходить к подстраницам.
...
12. **По схеме A:** локально `npm run build && pm2 restart [project]` — главная видна на основном домене
13. **По схеме B:** `git add . && git commit -m 'feat: homepage v1' && git push origin dev`. PM2 рестарт `[project]-dev` → главная видна на `dev.[domain]`
