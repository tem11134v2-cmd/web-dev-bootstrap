...
> Оркестрация: builder-sonnet; параллель: с 09 запрещена (обе правят app/layout.tsx) — строго последовательно; verifier: да — общий после пары 08+09 (валидаторы Rich Results/Yandex — часть его протокола)
>
> **08 и 09 оба правят `app/layout.tsx`** — в оркестраторном режиме выполняются строго последовательно (см. docs/orchestration.md).
...
1. `app/robots.ts`:
   ...
   ```
2. `app/sitemap.ts` — собрать все публичные страницы (статичные из docs/pages.md + статьи из `allPosts` (content-collections), если есть блог)
...
11. Верификация Яндекс Вебмастера — целиком в спеке 10: файл кладётся в `public/` (HTML-файлы внутри `app/` App Router не отдаёт). Здесь ничего не делаем
