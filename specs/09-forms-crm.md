...
12. Обновить `components/layout/Footer.tsx` — добавить ссылки на `/privacy/` и `/terms/` (страницы только что созданы, и кука-баннер из шага 7 уже на них ссылается)
...
15. `pnpm build` проходит, .env переменные не попали в клиентский бандл (не использовать `NEXT_PUBLIC_` для секретов!)
