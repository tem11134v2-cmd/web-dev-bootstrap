...
> Оркестрация: интерактив (оркестратор/человек), субагенту не отдаётся; параллель: нет; verifier: не нужен
...
Предполагается, что пользователь уже прошёл `_BUILD/HOW-TO-START.md` §0-§3 (установил mise + gh + git identity + ssh-keygen, создал папку и репо через `gh repo create --template`, открыл Claude Desktop в папке). Эта спека проверяет инструменты и пишет в память.
...
jq --version         # любая версия; нужен хукам .claude/hooks/* — без jq они fail-closed
```

Если чего-то нет — **не ставь сам**, попроси пользователя установить и подтвердить (`jq` на Mac — `brew install jq`). Claude не управляет глобальным тулчейном Mac.

**Windows-заметка:** если разработка ведётся на Windows — хуки `.claude/hooks/*.sh` требуют git-bash из Git for Windows (ставится вместе с Git); без него хуки не работают.
...
Это чтобы у других разработчиков и на GitHub-runner совпадали версии Node и pnpm. `mise` автоматически читает этот файл и подменяет версии при `cd` в папку.
