...
**Состав страниц (канон):** `/privacy` (политика ПДн), `/consent` (текст согласия — на неё ссылается чекбокс форм), `/offer` — только при приёме оплат на сайте. Все юр-страницы **индексируются** — `noindex` не ставить, их наличие = коммерческий фактор (`docs/seo.md` § 10).
...
> **Счётчики — после согласия.** Яндекс.Метрика / GA грузятся **только после** клика «Принять» в баннере; до согласия метрические cookie не ставятся. Реализация — `specs/10-analytics.md`.
...
- [ ] Счётчики аналитики грузятся только после согласия в баннере
