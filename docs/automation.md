...
| **`/security-audit`** | Перед первым прод-релизом; затем раз в месяц или после крупных фич | Полный security-аудит проекта (секреты, Server Actions, заголовки, зависимости, CI/CD, 152-ФЗ) с отчётом КРИТИЧНО/ВАЖНО/РЕКОМЕНДАЦИЯ. Только отчёт, код не меняет. Не путать со встроенной `/security-review` — та проверяет только текущий дифф. |

Подробное содержимое команд — в `.claude/commands/{handoff,resume,catchup,security-audit}.md`. Файлы можно править под свой проект — это обычные prompt-инструкции.
