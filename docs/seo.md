...
### hreflang (только мультиязычные сайты — `specs/optional/opt-i18n.md`)

- Каждая языковая версия перечисляет **все** версии + `x-default` (обычно = основной язык).
- В Next — `alternates.languages` в `metadata`; при next-intl alternates генерятся автоматически из конфига локалей — руками не дублировать.
- hreflang двусторонний: `/en/` ссылается на `/ru/` → `/ru/` обязан ссылаться на `/en/`, иначе пара игнорируется.
- Для одноязычного RU-сайта hreflang не нужен вовсе.
