...
**Делает человек** (GUI регистратора — чужой интерфейс, Claude туда не ходит). Claude готовит значения записей и проверяет распространение через `dig`. Подключение домена к серверу, делается один раз на домен.
