...
1. Читай `README.md` и `_BUILD/changelog.md` (история версий v2.0 →
   v2.2.x), чтобы понимать, что уже сделано.
2. `git log --oneline -10` и посмотри текущую ветку/теги.
3. Если есть `IDEAS.md` — прочти, предложи приоритизировать.
4. Спроси пользователя: «Что улучшаем в шаблоне?»
5. Работа: ветка `feature/{тема}`, commit, push, PR в `main`. После
   merge — новый semver-тег (v2.2.3, v2.3.0 и т.д.), запись в
   `_BUILD/changelog.md` (сверху, самая свежая версия).
6. **НЕ запускай спеки `specs/00-13`** — они для сайтов, которые
...
- **Rollback prod:** `scripts/rollback.sh <commit-hash>` — rolls prod back on the VPS (git reset + npm ci + build + pm2 restart). Then on Mac: `git revert <bad-commit> && git push origin main` — Actions redeploys the fix. For merge commits use `git revert -m 1 <hash>`.
