...
## v4.0 — 2026-07-17 · Большой апгрейд

Крупнейший релиз после v3.0: оркестраторный режим, встроенный браузер как штатный верификатор, design-first вход, актуализация стека под Next 16 / Biome 2 / Zod 4, распил forms-and-crm, SSH-канон, лайт-профиль и полевые уроки пяти боевых проектов. Канонические решения зафиксированы в едином наряде (DECISIONS-V4), файлы синхронизированы между собой.

- **Оркестраторный слой.** `.claude/agents/` с 4 ролями (builder-sonnet, builder-opus, verifier, integrator), новый `docs/orchestration.md` (волны, брифы, гейты, файлы-перекрёстки), команда `/orchestrate`, Task ledger в `project_state.md`. Интерактивный режим остаётся дефолтом для мелких задач; оркестратор — для крупных пластов (сайт целиком, rollout, recreate). Память пишет только оркестратор, субагентам read-only.
- **Встроенный браузер как штатный верификатор.** `docs/workflow.md` переписан вокруг мантры «Claude ВИДИТ сайт»: сам поднимает dev-сервер (`preview_start` + `.claude/launch.json`), скриншотит 375/768/1280, читает консоль и network. `opt-visual-recreate` v2 — на две вкладки встроенного браузера (донор + localhost, каждый вызов с явным tabId). Chrome-плагин остаётся только для залогиненных сессий реального Chrome (доноры за авторизацией).
- **Design-first вход.** Развилка «Источник дизайна» в `00-brief`: готовый макет → новая `specs/optional/opt-design-to-site.md` (забор из Claude Design через DesignSync, извлечение токенов, parity-check в браузере); референсы → новая `specs/optional/opt-design-from-references.md` (синтез паттернов, не копия); с нуля → классический путь. Плюс `docs/design-motion.md` против шаблонности (каталог интерактивных паттернов, правило анти-монотонности, бюджет движения) и `docs/ai-assets.md` (конвейер AI-изображений).
- **Починка обвязки.** Guard-хуки: jq обязателен (fail-closed вместо тихого пропуска), PowerShell-matcher, ужесточённый force-push regex, вывод в stdout, состояние в `.claude/state/` вместо `/tmp`, smoke-тест `test-hooks.sh`; typecheck-гейт перед push в `before-push.sh`.
- **Актуализация стека.** Next 16: `cacheComponents` вместо удалённого `experimental.ppr`, `await params` во всех код-сэмплах, Turbopack — дефолт без флага; канонический `next.config.ts` — в `docs/stack.md`. Biome 2 (`files.includes` с `!`-негациями, init через визард create-next-app). Zod 4 API (`z.email()`, `{ error }`). Установка content-collections через `@content-collections/*`-пакеты. `.npmrc` с `node-linker=hoisted` — обязательный шаг init. Turnstile ставится один раз — в спеке 09. Telegram-sink — голый fetch на Bot API.
- **Распил forms-and-crm.** `docs/forms-and-crm.md` ужат до ядра (архитектура multi-sink, Server Action, классификация, fallback); полные листинги каналов — в новом `docs/forms-sink-recipes.md`. **Email-sink (nodemailer, SMTP) — первый канал**: «почта сегодня, остальное когда попросят» — официальный дефолт. Fallback-лиды — в `LEADS_DIR` (`~/prod/{site}/shared/data` на VPS).
- **SSH-канон.** Одна политика в CLAUDE.md вместо трёх противоречащих формулировок («Claude не лезет» / «не ходит» / «Never подключаться»): Claude ходит на VPS ключом разработчика для настройки, подключения сайтов и read-only проверок, предупредив пользователя; человеку — только чужие GUI (DNS, оплата, верификации).
- **CLIENT-TODO.md.** Первоклассный артефакт в корне проекта (`specs/templates/client-todo-template.md`): доступы, материалы, подтверждения, верификации. Создаётся в 00, пополняется по триггеру из CLAUDE.md, пересылается клиенту целиком.
- **Лайт-профиль.** `_BUILD/templates/claude-md-lite.md` — для лендингов на 1–3 страницы, где полный bootstrap избыточен: сжатый стек, «истина дизайна», деплой-runbook в 5 шагов, мини CLIENT-TODO. Развилка описана в HOW-TO-START §1.
- **Полевые уроки 5 боевых проектов** внесены как факты: deploy одним job'ом без upload/download-artifact (квота хранилища артефактов валила деплой); `PORT` и `HOSTNAME=127.0.0.1` в env pm2-команд; после смены симлинка `pm2 delete && start`, не restart (кэширует resolved-путь); `gzip -t` + проверка `server.js` до переключения симлинка + healthcheck с откатом (обновлены `deploy-*.yml.example`); `shared/data` для лидов — деплой и cleanup не трогают; классы с префиксом `adv*` режутся адблоками; cyrillic-сабсет шрифта — чек в спеке 03; placeholder + show-флаги вместо выдуманных данных; ребейз только на `origin/main` после merge.
- **Дедуп и переезды.** HOW-TO-START ужат (~19KB: троблшутинг → указатель на `docs/troubleshooting.md`, домен → 5 строк + `docs/domain-connect.md`), добавлены 0-Windows и развилки профиля/дизайна. Шаблоны — в `_BUILD/templates/` (deploy-yml из v3/templates + deploy-readme из 01b + handoff-template из 12 + лайт-профиль); исторические ТЗ — в `_BUILD/archive/` с пометкой «архив». `_BUILD/v3/02-migrate-existing-project.md` остался на месте (hardcoded raw-URL).

> Ретро-запись: **2026-06-03/04** — цикл `opt-visual-recreate` (5 коммитов: переписка спеки по полевому прогону rus-campers.com, ревизии §F, три прохода самокритики) прошёл без changelog-записи. Восстановлено задним числом в v4.0; первоисточники — `_BUILD/archive/visual-recreate-plan.md` и `_BUILD/archive/algorithm-improvements.md`.

### Что НЕ затронуто

- Архитектура multi-sink (`Promise.allSettled`, `SinkSkipped`, fallback) — без изменений, распил только по файлам; email добавлен каналом, остальные не трогались.
- Push-based deploy как схема (releases/<sha> + симлинк current) — без изменений; поменялась механика внутри workflow (один job, tar вместо artifact+rsync, pm2 delete/start, healthcheck).
- `_BUILD/v3/02-migrate-existing-project.md` — не перемещён и не редактировался.
- Существующие сайты на v3.x продолжают работать; правки живут в шаблоне и применятся в новых проектах из template.

## v3.4 — 2026-06-04 · AmoCRM как first-class lead-sink

amoCRM доведён из «болванки в доке» до готового к подключению канала. До v3.4 шаблон `sendToCRM` под Amo лежал в `docs/forms-and-crm.md`, но (а) содержал баг прицепки примечания, (б) не имел инструкции по получению токена, (в) не умел маршрутизировать в воронку. Архитектура multi-sink не менялась — Amo как был третьим sink'ом рядом со Sheets/Telegram, так и остался; доведена именно amoCRM-болванка.

- **Фикс бага с примечанием.** `POST /api/v4/leads/complex` возвращает плоский массив `[{ id, contact_id, request_id, merged }]`, а шаблон читал `created._embedded?.leads?.[0]?.id` → `leadId` всегда `undefined`, примечание с текстом сообщения молча не прикреплялось (сделка создавалась, комментарий клиента терялся). Теперь `const created = (await res.json()) as Array<{ id: number }>; const leadId = created[0]?.id;`. Сверено с офиц. докой amoCRM (leads-api).
- **Опциональная маршрутизация в воронку/этап.** Добавлены `AMO_CRM_PIPELINE_ID` / `AMO_CRM_STATUS_ID` — заданы, сделка падает в конкретную воронку/этап; не заданы — в первый этап главной воронки (прежнее поведение). Тело сделки собирается через объект `lead`, в который условно домешиваются `pipeline_id`/`status_id`.
- **Новый раздел «Подготовка amoCRM (один раз)»** в `docs/forms-and-crm.md` — по образцу разделов Sheets/Telegram. Пошагово: создать внешнюю интеграцию → вкладка «Ключи и доступы» → сгенерировать **долгосрочный токен** (1 день–5 лет, для stateless-деплоя без БД — без OAuth refresh-ротации); `AMO_CRM_URL` без слэша (Kommo — `.kommo.com`); где взять ID воронки/этапа; предупреждение о безопасности токена. Закрывает пробел: для Sheets/Telegram пре-req был, для Amo — нет.
- **Примеры `.env`/`.env.example` обновлены** — CRM-блок переименован в «AmoCRM», добавлены `AMO_CRM_PIPELINE_ID`/`AMO_CRM_STATUS_ID` с пояснениями, в `.env.example` заведён закомментированный CRM-блок (раньше Amo-ключей там не было вовсе).
- **`specs/09-forms-crm.md` §11** — шаг CRM теперь ссылается на «Подготовка amoCRM (один раз)» и перечисляет ключи Amo (включая опциональную маршрутизацию), вместо общей фразы «long-lived integration token».
- **`README.md`** H1 → v3.4.

### Что НЕ затронуто

- Архитектура multi-sink (`Promise.allSettled` поверх `allSinks`, `SinkSkipped` skip-vs-fail, JSON-fallback) — без изменений. Amo остаётся одним из трёх sink'ов.
- Шаблоны Bitrix24 и других CRM, sinks Sheets/Telegram, Server Action, клиентские формы, Turnstile — не трогались.
- Существующие сайты: правка живёт в доке/спеке шаблона. Сайт, где Amo уже подключён со старым багом, чинится точечно — заменой чтения ответа `/leads/complex` на `created[0]?.id` в `lib/sinks/crm.ts` (одна строка).
...
- **`_BUILD/HOW-TO-START.docx` удалён.** Был зеркалом `.md` через pandoc, нужен был для оффлайн-чтения. Регенерация .docx висела как outstanding с v3.0 (PR #16/#18 регенерили старую версию, после v3.1 unified rewrite docx так и остался без обновления). Решено: один источник истины — `.md` в репо, читается с GitHub без локального клона. Упоминание `_BUILD/HOW-TO-START.md (+.docx)` в `README.md` обрезано, outstanding-таски в `.claude/memory/project_state.md` про регенерацию docx удалены.
...
- `CLAUDE.md` BOOTSTRAP META vs `## Rules` про ветки — внутреннее противоречие (bootstrap-mode vs site-mode) отложено как отдельное обсуждение. **Закрыто в v3.3** через site-mode guard в шапке `## Rules` (без расщепления файла на два).
