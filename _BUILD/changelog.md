...
## v3.5 — 2026-07-13 · Бизнес-память + security-минимум

Два пробела закрыты по итогам разбора внешних материалов по работе с Claude Code (июль 2026): (а) память проекта покрывала только dev-контекст (решения, фидбек, инциденты) — коммерческие наблюдения, ради которых сайты и делаются, копились нигде; (б) security-проверка существовала только в форме общих упоминаний в доках — не было ни команды полного аудита, ни pre-launch gate. Архитектура памяти и хуков не менялась — добавлены новые файлы в существующие механизмы.

- **`.claude/memory/business.md`** — новый файл памяти: коммерческие факты, узнанные по ходу проекта (изменения цен, метрики воронки, возражения клиентов, сработавшие офферы). Явное разграничение с `docs/spec.md` (статика от заказчика на старте) и `references.md` (внешние ID). Формат записи: заголовок-утверждение + цифры + источник + How to apply + дата, примеры в комментариях по образцу `feedback.md`. Триггер добавлен в `CLAUDE.md § Memory triggers`, строка — в `memory/INDEX.md`.
- **Правило заголовков-утверждений** зафиксировано в `memory/INDEX.md`: заголовок записи — утверждение («Лиды с квиза конвертятся в 2 раза лучше формы»), не категория («конверсия») — релевантность видна по заголовку без открытия файла.
- **`/security-audit`** — новая slash-команда (`.claude/commands/security-audit.md`): полный аудит проекта в 8 шагов под наш стек (секреты и git-история, Server Actions/Zod/Turnstile, заголовки Caddy, `pnpm audit`, утечки, пиновка Actions, VPS, 152-ФЗ) с отчётом КРИТИЧНО/ВАЖНО/РЕКОМЕНДАЦИЯ + файл:строка + фикс. Режим «только отчёт» — код не меняет, фиксы отдельным шагом по одному. В шапке команды явно разведено со встроенной `/security-review` (та проверяет только текущий дифф, не весь проект). Задокументирована в `docs/automation.md` (таблица — теперь четыре команды).
- **`docs/deploy-checklist.md`** — pre-launch gate из 7 секций (секреты/доступы, формы/лиды, транспорт/сервер, 152-ФЗ, SEO, перфоманс, первые 24 часа после деплоя). Чек-лист — только индекс со ссылками на источники истины (`forms-and-crm.md`, `server-add-site.md`, `legal-templates.md` и т.д.), контент не дублирует (правило дедупликации docs/). Строка добавлена в `docs/INDEX.md` и в KB pointers `CLAUDE.md`.
- **`CLAUDE.md § Testing`** — новая строка: перед ПЕРВЫМ прод-релизом обязательны `/security-audit` + прогон `docs/deploy-checklist.md`.
- **`README.md`** H1 → v3.5.

### Что НЕ затронуто

- Механизмы памяти (`/resume`, `/handoff`, `/catchup`, хуки session-start/stop-reminder) — без изменений; `business.md` — просто ещё один файл в существующей схеме.
- Существующие memory-файлы (`decisions.md`, `feedback.md`, `lessons.md`, `pointers.md`, `references.md`, `project_state.md`) — не тронуты.
- Стек, спеки, скрипты, деплой — без изменений. Существующие сайты ничего не теряют: при желании `business.md` и команду можно скопировать в них руками из шаблона.
