...
Одна **интерактивная** Claude-сессия = одна задача (одна спека). Параллельные интерактивные сессии на **ОДНУ папку проекта** по-прежнему запрещены — они не видят друг друга и поломают `.claude/memory/project_state.md`. **Оркестраторный режим с субагентами — штатный и дефолтный для крупных пластов**: получив задачу уровня «целая спека 02–12, rollout страниц, recreate/redesign, 3+ однотипных страниц», Claude **сам** входит в режим оркестратора (действует как `/orchestrate`: план волны + один батч-вопрос) — команду ждать не нужно. `/orchestrate` — явный принудительный вход для средних случаев; точечные правки и микро-фиксы — интерактив. Оркестратор раздаёт брифы субагентам и один пишет память (субагентам она read-only). Правила — `docs/orchestration.md`. Последовательные сессии — норма:
