...
|---|---|---|
| **`/orchestrate`** | Явное принуждение к волне. Для крупного пласта — сайт целиком (02 → 12), rollout всех страниц, recreate/redesign — Claude войдёт в оркестраторный режим **сам**, команду вызывать не обязательно | Включит оркестраторный режим: план волны, один батч-вопрос тебе (ключи/доступы/выборы), брифы субагентам, приём отчётов, verifier на крупные пласты. Правила — `docs/orchestration.md`. Для точечных правок не нужен — интерактив дешевле. |
