...
### Как это работает на практике (один нюанс с permissions)

Claude Desktop, открытый в папке проекта (`~/projects/{site}/`), по умолчанию имеет доступ **только к этой папке**. Промт ниже просит Claude прочитать ТЗ из bootstrap-папки — это **другая папка**, и Claude Desktop при первой попытке доступа покажет окно с разрешением:

> *Allow access to `~/ClaudeCode/web-dev-bootstrap`?*  →  **Allow once / Allow always / Deny**

Нажми **«Allow always»** — это разовая операция на этот проект. После того как доступ выдан, Claude читает ТЗ-2, копирует канонические файлы из bootstrap'а (`biome.json.example`, hooks, slash-команды, deploy-workflow templates) и проводит миграцию без дополнительных вопросов.
...
При первом обращении к файлу — нажми «Allow always» в permission-окне. Дальше Claude:

1. Прочитает ТЗ-2 (~900 строк, разбитое на 6 этапов)
2. Сделает pre-flight: определит текущую версию проекта (по `_BUILD/changelog.md` или признакам), проверит `git status`, поставит `pre-v3-migration-YYYYMMDD` тег для отката
3. Покажет тебе **персональный план миграции** под этот конкретный проект (что в проекте есть, что миграция затронет) — жди ОК
...
### Альтернативы (если не хочешь давать permission на bootstrap-папку)

Practically — permission всё равно понадобится в какой-то момент: ТЗ-2 содержит `cp ~/ClaudeCode/web-dev-bootstrap/biome.json.example ...` и аналогичные команды для копирования канонических файлов. Без доступа к bootstrap-папке миграция не пройдёт.

Если по каким-то причинам не хочешь выдавать `Allow always` — единственная рабочая альтернатива: **сначала скопировать всё нужное** руками в проект (ТЗ-2 + все упомянутые в нём файлы из `~/ClaudeCode/web-dev-bootstrap/...`), потом запустить миграцию. Это много ручной работы и легко что-то пропустить — рекомендую `Allow always` как стандартный путь.

Подробности всех 6 этапов — в `_BUILD/v3/02-migrate-existing-project.md`.
